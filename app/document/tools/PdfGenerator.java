...
public class PdfGenerator extends PdfPageEventHelper {
	...
	// Verse styling
	String[] verseTypes = { verseType.Verse.name(), verseType.Chorus.name(), verseType.Bridge.name(),
			verseType.Intro.name(), verseType.Ending.name() };
			...
			if (isThereSpaceForColumns && useColumns) {
				...
				if (!splitSingleSong && (idxPlusOne < numberOfSongs)) {
					...
					if (nextSongNumberOfLines < (maxLineNumber - 1) && (nextSongLongestLine < maxCharLenght)) {
						...
						final Chunk chunk2 = new Chunk(nextSong.getName(), fonts.BOLD).setLocalDestination(nextSong
								.getName());
	...
	private int createSongColumns(List<? extends PdfPrintable> printObject, boolean useColumns,
			boolean excludePageOfContent) throws IOException, DocumentException {
		total = writer.getDirectContent().createTemplate(30, 16);
		...
		for (int i = 0; i < numberOfSongs; i++) {
			...
			for (int j = 0; j < maxNumberOfSongsPerPage; j++) {
				...
				// if second column active, and I don't have any more place in it or if any of the song has too long
				// line
				if (secondColumnActive || (writtenLines >= maxLinesPerColumn)) {
					...
					if (writtenLines >= maxLinesPerColumn) {
						...
						boolean previousSongHasTooLongLine = (previousSong.getLongestLine() > maxCharLenght) ? true
								: false;
				...
				if ((j != 0)
						&& ((songNumberOfLines + writtenLines) <= maxLinesPerColumn)
		...
		if (!excludePageOfContent) {
			...
			tocStartPage = this.writer.getPageNumber();
			...
			ColumnText.showTextAligned(total, Element.ALIGN_LEFT,
					new Phrase(String.valueOf(writer.getPageNumber() - numberOfTocPages)), 2, 2, 0);
	...
	private static void reorderToC(PdfReader reader, int tocStartPage, String outputPdfPath)
	...
	public static void writeListContent(String outputPdfPath, List<? extends PdfPrintable> songPrintObjects,
			boolean useColumns, boolean excludePageOfContent) {
		PdfGenerator pdfGenerator;
		try {
			int tocStartPage = 0;
			if (!useColumns) {
				// SSPP Engine (Single Song Per Page)
				...
				// MCS Engine (Multi Column Song)
				pdfGenerator = new PdfGenerator(outputPdfPath, true);
				tocStartPage = pdfGenerator.createSongColumns(songPrintObjects, useColumns, excludePageOfContent);
			}
