...
public class HtmlBuilder {
	...
	/*
	 * <font color="#c5c5c5">" + "Competitor ID: " + "<font color=\"#47a842\" face=\"monospace\">" + songLyrics +
	 * "</font>"
	 */
	public static String androidViewHtmlDecorator(String prependStr, String orgStr, String appendStr) {
	...
	public static String buildHtmlFromSongLyrics(String songlyrics) {
		...
		for (String line : songlyrics.split("\\r?\\n")) {
			...
			if (lineStartsWithBrace || ArrayHelper.stringContainsItemFromList(line, verseTypes)) {
				...
				// trim line
				songLyricsHtml.append(androidViewHtmlDecorator(verseTypeOpenTag, line.trim(), verseTypeCloseTag));
			} else if (LineTypeChecker.isChordLine(line)) {
				// CHORD STYLING
				// BLUE line
				songLyricsHtml.append(androidViewHtmlDecorator(chordOpenTag, line, chordCloseTag));
			} else {
				// STANDARD STYLING
				// line
				songLyricsHtml.append(androidViewHtmlDecorator(verseOpenTag, line, verseCloseTag));
			}
		}
	...
	public static String buildHtmlSongButtonLinks(List<String> songLyricsIds, String songName) {
	...
	public static String buildHtmlVideoButtonLink(String videoLink) {
		if (videoLink != null && URLParamEncoder.isUrl(videoLink)) {
			StringBuilder sb = new StringBuilder("<button type=\"button\" class=\"ytlinkbutton\" id=\"");
			sb.append(videoLink);
			sb.append("\" onclick=\"openYtLink(this.id)\"><i class=\"fa fa-play-circle fa-lg\"></i></button>");
			return sb.toString();
		} else {
			return "";
		}
