...
public class DocumentWriter {
	...
	public void writeSong(XWPFDocument document, String songTitle,
			String songLyrics, int songTotalNumber, boolean isLast) {
	...
	public File newSongbookWordDoc(String filename,
		...
		Logger.trace("Fonts - title:" + songTitleFont + " lyrics: "
				+ songLyricsFont);
		Logger.trace("Sizes - title:" + songTitleSize + " lyrics: "
				+ songLyricsFontSize);
		...
		for (int i = 0; i < songPrintObjects.size(); i++) {
			...
			Logger.trace("Now exporting songId: "
					+ songPrintObjects.get(i).getSong().id);
			...
			writeSong(document, s.getSong().songName, songLyrics,
					songTotalNumber, isLast);
		}
